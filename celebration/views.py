...
def index(request):
    ...
    return render(
        request,
        'index.html',
        context={'num_companies':num_companies,'num_branch':num_branch,'num_customers':num_customers,'num_occassions':num_occassions, 
...
#    xls = xls.drop(['Order TKN By', 'CHKD By', 'Produced Branch', 'Produced By', 'CHKD By'], axis=1)
    ...
    # TARGET COMPANY TEMPORARY
    target_company = 'Misterbaker LLC'
    # TARGET COMPANY TEMPORARY
    ...
    for i in dict_list:
        ...
        q = Company.objects.filter(company_name__iexact=target_company)
        if q.count() == 0:
            new_company = Company(company_name = target_company)
            new_company.save()
        ...
        try:
            ...
            if q.count() == 0:
                ...
                new_order.Company = Company.objects.get(company_name=target_company)
